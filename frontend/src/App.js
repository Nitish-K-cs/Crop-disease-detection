  ...
  const sendFile = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        const rawClass = (res.data.class ?? res.data.predicted_class ?? "").toString();
        const conf =
    ...
    <div className="root">
      ...
      <nav className="navbar">
        <div className="navbar-inner">
          ...
          <div className="navbar-links">
            <a href="#predict" className="nav-link nav-active">Predict</a>
          </div>
        </div>
      </nav>
      ...
      <main className="page">
        ...
        <section className="upload-section" id="predict">
          ...
          <p className="upload-subtitle">Detect plant problems with AI and protect your crops.</p>
          ...
          <div className="card two-column">
              ...
              <label className="uploadAreaLabel">
                {previewUrl ? (
                  ...
                  <div className="uploadPlaceholder">
                    <svg className="uploadIcon" viewBox="0 0 24 24" fill="none" aria-hidden>
                      <path d="M16 17L12 13L8 17" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                      <path d="M12 13V21" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                      <path d="M20 18C20 19.6569 18.6569 21 17 21H7C5.34315 21 4 19.6569 4 18C4 16.3431 5.34315 15 7 15H8.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    ...
                    <div className="uploadSubtitle">(JPG, PNG, up to 2MB). You can also use History below.</div>
                  </div>
                )}
                <input type="file" accept="image/*" onChange={onFileChange} className="fileInput" />
              </label>

              <div className="controls">
                <button onClick={sendFile} disabled={isLoading || !selectedFile || data} className={`button predictButton ${isLoading || !selectedFile || data ? "buttonDisabled" : ""}`}>
                  {isLoading && <Loader />} {isLoading ? "Analyzing Leaf..." : "Classify Disease"}
                </button>

                <button onClick={onClear} className="clearButton" disabled={!selectedFile && !data}>Clear</button>
              </div>
            ...
            <div className="right-col">
              ...
              {rawClass ? renderResult(rawClass, confidence, previewDataUrl) : <div className="placeholderResult">No prediction yet. Upload an image and click "Classify Disease".</div>}
            </div>
          </div>
          ...
          <div className="historyWrap">
            ...
            {history.length === 0 ? <div className="historyEmpty">No previous predictions yet.</div> : (
              <div className="historyList">
                {history.map((item) => (
                  <button key={item.id} className="historyItem" onClick={() => loadHistoryItem(item)}>
                    <div className="histThumb">{item.image ? <img src={item.image} alt="" /> : <div className="noThumb">No image</div>}</div>
                    <div className="histMeta">
                      <div className="histTitle">{item.crop} — {item.disease}</div>
                      <div className="histConf">{(item.confidence * 100).toFixed(2)}%</div>
                      <div className="histTime">{new Date(item.timestamp).toLocaleString()}</div>
                    </div>
                  </button>
                ))}
              </div>
        ...
        <footer className="footer">
          <small>Model Endpoint: {API_URL} • Built for project submission</small>
        </footer>
      </main>
